...
node::node(node_id new_id, uint32_t memsz, boost::shared_ptr<router> new_rt,
           boost::shared_ptr<channel_alloc> new_vca,
           boost::shared_ptr<tile_statistics> st,
           boost::shared_ptr<vcd_writer> v,
           logger &l, boost::shared_ptr<random_gen> ran) throw()
...
void node::add_ingress(node_id src, boost::shared_ptr<ingress> ingress) throw(err) {
...
void node::add_egress(node_id dst, boost::shared_ptr<egress> egress) throw(err) {
...
boost::shared_ptr<ingress> node::get_ingress_from(node_id src) throw(err) {
    assert(src.is_valid());
    if (ingresses.find(src) == ingresses.end()) {
...
boost::shared_ptr<egress> node::get_egress_to(node_id dst) throw(err) {
    assert(dst.is_valid());
    if (egresses.find(dst) == egresses.end()) {
...
boost::shared_ptr<router> node::get_router() throw() { return rt; }

boost::shared_ptr<channel_alloc> node::get_channel_alloc() throw() { return vc_alloc; }

boost::shared_ptr<pressure_tracker> node::get_pressures() throw() { return pressures; }
...
void node::connect_from(const string &port_name,
    ...
    boost::shared_ptr<ingress> ingr =
        boost::shared_ptr<ingress>(new ingress(dst_id, src->get_id(), vq_ids,
                                        flits_per_queue, bw_to_xbar,
                                        vc_alloc, pressures, stats, vcd, log));
    egress_id src_id(src->get_id(), src_port_name);
    boost::shared_ptr<egress> egr =
        boost::shared_ptr<egress>(new egress(src_id, ingr, src->get_pressures(),
                                      link_bw, log));
