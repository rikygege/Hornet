...
public:
    tile_statistics(const uint64_t &system_time, const uint64_t &start_time,
                    boost::shared_ptr<flow_rename_table> flow_renames) throw();
...
private:
    ...
    typedef boost::tuple<egress_id, egress_id, unsigned> sub_link_id;
    ...
    typedef boost::tuple<node_id, node_id> cxn_id;
...
public:
    ...
    uint64_t get_received_packet_count() const throw();
    void add(uint32_t id, boost::shared_ptr<tile_statistics> new_stats) throw();
    void reset() throw();
    void start_sim() throw();
    void end_sim() throw();
    inline uint64_t  get_stats_time(uint32_t index) const throw();
    boost::shared_ptr<tile_statistics> get_tile_stats(uint32_t index) const throw();
    inline void add_aux_statistics(boost::shared_ptr<aux_statistics> stats) { aux_stats.push_back(stats); }
public:
    friend ostream &operator<<(ostream &, const system_statistics &);
private:
    typedef map<uint32_t, boost::shared_ptr<tile_statistics> > tile_stats_t;
    typedef vector<boost::shared_ptr<aux_statistics> > aux_stats_t;
...
inline uint64_t system_statistics::get_stats_time(uint32_t index) const throw()  {
    ...
    const boost::shared_ptr<tile_statistics> ts = ti->second;
    return ts->get_system_time() - ts->get_start_time();
}

inline boost::shared_ptr<tile_statistics> system_statistics::get_tile_stats(uint32_t index) 
   const throw() {
      tile_stats_t::const_iterator ti = tile_stats.find(index);
      assert(ti != tile_stats.end());
      const boost::shared_ptr<tile_statistics> ts = ti->second;
