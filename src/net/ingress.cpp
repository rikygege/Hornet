...
ingress::ingress(const ingress_id &new_id, const node_id &new_src_node_id,
                 ...
                 boost::shared_ptr<channel_alloc> vca,
                 boost::shared_ptr<pressure_tracker> pt,
    ...
    for (set<virtual_queue_id>::const_iterator i = vq_ids.begin();
        ...
        if (vqs.find(*i) != vqs.end())
            throw err_duplicate_queue(parent_id.get_numeric_id(),
                                      i->get_numeric_id());
        boost::shared_ptr<virtual_queue>
...
void ingress::add_queue(boost::shared_ptr<virtual_queue> vq) throw(err) {
    ...
    next_hops[vq->get_id().get<1>()] =
        boost::make_tuple(node_id(), virtual_queue_id());
}
