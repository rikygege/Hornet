...
static uint32_t read_word(boost::shared_ptr<ifstream> in) throw(err) {
...
static double read_double(boost::shared_ptr<ifstream> in) throw(err) {
...
static string read_string(boost::shared_ptr<ifstream> in) throw(err) {
    char buf[256];
...
static void read_mem(uint8_t *ptr, uint32_t num_bytes,
                     boost::shared_ptr<ifstream> in) throw(err) {
    assert(ptr);
...
sys::sys(const uint64_t &new_sys_time, boost::shared_ptr<ifstream> img,
    ...
    for (unsigned i = 0; i < num_nodes; ++i) {
        ...
        boost::shared_ptr<set_bridge_channel_alloc> vca =
            boost::static_pointer_cast<set_bridge_channel_alloc>(b_vca);
        ...
        case CORE_MCPU: {
            ...
            //boost::shared_ptr<memory> instruction_memory = boost::shared_ptr<memory>();
            ...
            boost::shared_ptr<privateSharedMSI> new_data_memory = 
                boost::shared_ptr<privateSharedMSI>(new privateSharedMSI(id, t->get_time(), 
                                                                  t->get_statistics(), log, ran, new_cat, cfg));
            //boost::shared_ptr<privateSharedMSI> new_instruction_memory = 
            //    boost::shared_ptr<privateSharedMSI>(new privateSharedMSI(id, t->get_time(), 
            //                                                     t->get_statistics(), log, ran, new_cat, cfg));
    ...
    for (unsigned i = 0; i < num_routes; ++i) {
        ...
        if (prev_n == 0xffffffffUL) { // program the bridge
            ...
            boost::shared_ptr<set_bridge_channel_alloc> vca =
                boost::static_pointer_cast<set_bridge_channel_alloc>(br_vcas[cur_n]);
            ...
            for (uint32_t n = 0; n < num_nodes; ++n) {
                ...
                boost::shared_ptr<set_channel_alloc> vca =
                    boost::static_pointer_cast<set_channel_alloc>(n_vcas[cur_n]);
            ...
            boost::shared_ptr<set_router> r =
                boost::static_pointer_cast<set_router>(node_rts[cur_n]);
