...
public:
    tile(const tile_id &id, const uint32_t num_tiles,
         ...
         boost::shared_ptr<flow_rename_table> flow_renames, logger &log) throw();
    void add(boost::shared_ptr<pe> p) throw();
    void add(boost::shared_ptr<bridge> p) throw();
    void add(boost::shared_ptr<node> p) throw();
    void add(boost::shared_ptr<arbiter> a) throw();
    const uint64_t &get_time() const throw();
    boost::shared_ptr<tile_statistics> get_statistics() const throw();
    boost::shared_ptr<id_factory<packet_id> > get_packet_id_factory() const throw();
...
private:
    ...
    typedef vector<boost::shared_ptr<arbiter> > arbiters_t;
private:
    const tile_id id;
    uint64_t time;
    pes_t pes;
    ...
    boost::shared_ptr<id_factory<packet_id> > pid_factory;
    logger &log;
};
