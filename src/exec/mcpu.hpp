...
public:
    explicit mcpu(  const pe_id                         &id, 
                    ...
                    boost::shared_ptr<dram>                    backingDRAM_data,
...
private:
    ...
    void execute(boost::shared_ptr<instr> i) throw(err);
...
private: // instruction memory hierarchy
    boost::shared_ptr<memory> i_memory;

private: // instruction/data interface
    
    boost::shared_ptr<memoryRequest> pending_request_instruction;
    boost::shared_ptr<memoryRequest> pending_request_data;
    ...
    */
    ...
    boost::shared_ptr<instr> instruction_fetch_complete(uint32_t pc);

    // data {read,write}{fetch,complete} interface
    void data_complete();
    void data_complete_helper(boost::shared_array<uint32_t>, uint32_t);
    ...
    inline void close_memory_op() {
        pending_request_data = boost::shared_ptr<memoryRequest>(); // NULLIFY
...
protected: // memory hierarchy processing & inherited members
    inline boost::shared_ptr<memory> nearest_memory_instruction() {
        return i_memory;
    }
    inline boost::shared_ptr<memory> nearest_memory_data() {
        return m_memory;
    }
