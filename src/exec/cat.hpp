...
public:
    ...
    virtual void request(boost::shared_ptr<catRequest> req) = 0;
...
protected:
    inline void set_req_status(boost::shared_ptr<catRequest> req, catReqStatus_t status) { req->m_status = status; }
    inline void set_req_home(boost::shared_ptr<catRequest> req, uint32_t home) { req->m_home = home; }
    inline maddr_t req_maddr(boost::shared_ptr<catRequest> req) { return req->m_maddr; }
    inline uint32_t req_sender(boost::shared_ptr<catRequest> req) { return req->m_sender; }
...
public:
    ...
    virtual void request(boost::shared_ptr<catRequest> req);
    virtual void tick_positive_edge();
    virtual void tick_negative_edge();

protected:
    typedef struct {
        boost::shared_ptr<catRequest> request;
