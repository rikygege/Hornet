...
mcpu::mcpu( const pe_id                         &new_id, 
            ...
            uint32_t                            stack_ptr,
            boost::shared_ptr<id_factory<packet_id> >  pif,
            boost::shared_ptr<tile_statistics>         new_stats,
            logger                              &l,
            boost::shared_ptr<random_gen>              r,
            boost::shared_ptr<memory>                  instruction_memory,
            boost::shared_ptr<memory>                  data_memory,
            boost::shared_ptr<dram>                    backingDRAM,
            ...
            :   core(                           new_id, 
                ...
                pending_request_instruction(boost::shared_ptr<memoryRequest>()),
                pending_request_data(boost::shared_ptr<memoryRequest>()),
...
inline uint32_t read_word_temp(boost::shared_ptr<ifstream> in) throw(err) {
...
void mcpu::tick_positive_edge_memory() throw(err) {
    if (m_memory != boost::shared_ptr<memory>()) {
        m_memory->tick_positive_edge();
    }
    /*if (i_memory != boost::shared_ptr<memory>()) {
...
void mcpu::execute() {
    if (running) { 
        ...
        data_complete();
        boost::shared_ptr<instr> i = instruction_fetch_complete(pc);
...
boost::shared_ptr<instr> mcpu::instruction_fetch_complete(uint32_t pc) {
    if (enable_memory_hierarchy) {
        if (!pending_data_memory_request() &&
            ...
            pending_request_instruction = boost::shared_ptr<memoryRequest>(); // reset to null
            return boost::shared_ptr<instr> (new instr(raw));
        }
        if (!pending_data_memory_request() && // TODO: serialize data/instr for now (fix later)
            ...
            boost::shared_ptr<memoryRequest> read_req(new memoryRequest(maddr, 1));
            nearest_memory_instruction()->request(read_req);
            pending_request_instruction = boost::shared_ptr<memoryRequest>(read_req);
        }
        return boost::shared_ptr<instr>();
    } else {
        uint32_t rdata;
        ...
        return boost::shared_ptr<instr> (new instr(rdata));
    }
}
...
void mcpu::data_complete_helper(boost::shared_array<uint32_t> req_data, uint32_t address) {
    uint64_t m;
...
void mcpu::data_fetch_read( const uint32_t &addr, 
    ...
    if (enable_memory_hierarchy) {
        ...
        pending_request_data = boost::shared_ptr<memoryRequest> (new memoryRequest(form_maddr(addr), words));
        nearest_memory_data()->request(pending_request_data);
    } else {
        uint32_t * read_data_inner = new uint32_t[words];
        boost::shared_array<uint32_t> read_data(read_data_inner);
...
void mcpu::data_fetch_write(    const uint32_t &addr, 
    ...
    boost::shared_array<uint32_t> write_data(write_data_inner);
    if (bytes != 8) {
        assert(bytes <= 4);
    ...
    if (enable_memory_hierarchy) {
        pending_request_data = boost::shared_ptr<memoryRequest> (new memoryRequest(form_maddr(addr), words, write_data));
        nearest_memory_data()->request(pending_request_data);
    } else {
...
void mcpu::execute(boost::shared_ptr<instr> ip) throw(err) {
    instr i = *ip;
    //cout << "[mcpu " << get_id() << "] "
