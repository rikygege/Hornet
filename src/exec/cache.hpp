...
typedef struct {
    ...
    boost::shared_ptr<void> coherence_info;
...
public:
    ...
    explicit cacheRequest(maddr_t maddr, cacheReqType_t request_type, 
                 ...
                 boost::shared_array<uint32_t> data_to_write = boost::shared_array<uint32_t>(), 
                 boost::shared_ptr<void> coherence_info_to_write = boost::shared_ptr<void>());

    ~cacheRequest();

    inline cacheReqStatus_t status() { return m_status; }

    inline boost::shared_ptr<cacheLine> line_copy() { return m_line_copy; }
    inline boost::shared_ptr<cacheLine> victim_line_copy() { return m_victim_line_copy; }
    ...
    inline void reset() { assert(m_status != CACHE_REQ_WAIT); 
                          ...
                          m_line_copy = boost::shared_ptr<cacheLine>();
                          m_victim_line_copy = boost::shared_ptr<cacheLine>(); }
...
private:
    ...
    boost::shared_ptr<cacheLine> m_line_copy;
    boost::shared_ptr<cacheLine> m_victim_line_copy;
    boost::shared_ptr<void> m_coherence_info_to_write;
    boost::shared_array<uint32_t> m_data_to_write;
...
class cache {
public:
    typedef boost::shared_ptr<void> (*helperCopyCoherenceInfo) (boost::shared_ptr<void>);
    typedef bool (*helperIsHit) (boost::shared_ptr<cacheRequest>, cacheLine&, const uint64_t&);
    typedef void (*helperReserveLine) (cacheLine&);
    typedef bool (*helperCanEvictLine) (cacheLine&, const uint64_t&);
    typedef uint32_t (*helperReplacementPolicy) (vector<uint32_t>&, cacheLine const*, const uint64_t&, boost::shared_ptr<random_gen> ran);

    cache(uint32_t cache_level, uint32_t numeric_id, const uint64_t &system_time, 
          boost::shared_ptr<tile_statistics> stats, logger &log, boost::shared_ptr<random_gen> ran, 
    ...
    void print_contents();
    void request(boost::shared_ptr<cacheRequest> req);
...
private:
    ...
    typedef struct {
        ...
        boost::shared_ptr<cacheRequest> request;
    ...
    boost::shared_ptr<cacheLine> copy_cache_line(const cacheLine &line);
    ...
    set<boost::tuple<uint32_t/*idx*/, uint32_t/*ways*/> > m_lines_to_evict; /* and leave it empty */
    set<boost::tuple<uint32_t/*idx*/, uint32_t/*way*/, maddr_t> > m_lines_to_evict_and_reserve; /* evict and reserve it for another line */
