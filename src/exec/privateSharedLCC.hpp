...
public:
    ...
    virtual void request(boost::shared_ptr<memoryRequest> req);
    virtual void tick_positive_edge();
    virtual void tick_negative_edge();

    /* set stats */
    inline void set_per_tile_stats(boost::shared_ptr<privateSharedLCCStatsPerTile> stats) { m_stats = stats; }
    inline bool stats_enabled() { return (m_stats != boost::shared_ptr<privateSharedLCCStatsPerTile>()); }
    inline boost::shared_ptr<privateSharedLCCStatsPerTile> stats() { return m_stats; }
    ...
    typedef struct {
        ...
        boost::shared_ptr<uint64_t> synched_expiration_time; /* only used in idealized timestamp logic */
        boost::shared_ptr<uint64_t> first_read_time_since_last_expiration;  /* this is supposed to be updated on the cache, but for now assumes thie information is kept somehow */

        /* only used in L2 with some timestamp logic */
        boost::shared_ptr<bool> in_large_mode;
        boost::shared_ptr<uint64_t> timestamp_delta_small; 
        boost::shared_ptr<uint64_t> timestamp_delta_large; 
    ...
    typedef struct {
        ...
        uint32_t word_count; 
        maddr_t maddr;
        boost::shared_array<uint32_t> data;

        uint64_t timestamp;
        boost::shared_ptr<uint64_t> synched_timestamp;
    ...
    typedef struct {
        uint32_t sender;
        uint32_t receiver;
        boost::shared_ptr<dramRequest> req;
        bool did_win_last_arbitration;
    } dramMsg;
...
private:
    ...
    typedef struct {
        toL1Status status;
        boost::shared_ptr<memoryRequest> core_req;
        boost::shared_ptr<catRequest> cat_req;
        boost::shared_ptr<cacheRequest> l1_req;
        boost::shared_ptr<coherenceMsg> lcc_req;
        boost::shared_ptr<coherenceMsg> lcc_rep;

        /* for stats */
        uint64_t requested_time;

        /* for performance */
        boost::shared_ptr<message_t> net_msg_to_send;
    ...
    typedef struct {
        ...
        boost::shared_ptr<coherenceMsg> lcc_read_req;
        boost::shared_ptr<coherenceMsg> lcc_write_req;
        boost::shared_ptr<cacheRequest> l2_req;
        boost::shared_ptr<coherenceMsg> lcc_rep;
        boost::shared_ptr<dramMsg> dram_req;
        boost::shared_ptr<dramMsg> dram_rep;
    ...
    typedef map<maddr_t, boost::shared_ptr<toL1Entry> > toL1Table;
    typedef map<maddr_t, boost::shared_ptr<toL2Entry> > toL2Table;
    typedef map<maddr_t, boost::shared_ptr<toDRAMEntry> > toDRAMTable;
    ...
    uint64_t get_expiration_time(boost::shared_ptr<cacheLine> line);
    ...
    cache* m_l1;
    cache* m_l2;
    boost::shared_ptr<cat> m_cat;

    boost::shared_ptr<privateSharedLCCStatsPerTile> m_stats;
    ...
    /* scheduler queues */
    vector<boost::shared_ptr<memoryRequest> > m_core_port_schedule_q; 

    /* scheduler queues for requests in to l2 work table */
    vector<boost::shared_ptr<coherenceMsg> > m_to_l2_req_schedule_q;
#if 0
    vector<boost::shared_ptr<coherenceMsg> > m_to_l2_read_req_schedule_q;
    vector<boost::shared_ptr<coherenceMsg> > m_to_l2_write_req_schedule_q;
#endif

    vector<boost::shared_ptr<dramMsg> > m_to_dram_req_schedule_q;

    /* scheduler queues for sending messages to the network */
    map<uint32_t/* per channel*/, vector<boost::shared_ptr<message_t> > > m_to_network_schedule_q;

    /* to-memory request scheduler */
    vector<boost::shared_ptr<catRequest> > m_cat_req_schedule_q;
    vector<boost::shared_ptr<cacheRequest> > m_l1_read_req_schedule_q;
    vector<boost::shared_ptr<cacheRequest> > m_l1_write_req_schedule_q;
    vector<boost::shared_ptr<cacheRequest> > m_l2_read_req_schedule_q;
    vector<boost::shared_ptr<cacheRequest> > m_l2_write_req_schedule_q;
    vector<boost::shared_ptr<dramRequest> > m_dram_req_schedule_q;
