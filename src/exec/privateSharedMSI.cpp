...
static boost::shared_ptr<void> cache_copy_coherence_info(boost::shared_ptr<void> source) { 
    boost::shared_ptr<privateSharedMSI::cacheCoherenceInfo> ret
        (new privateSharedMSI::cacheCoherenceInfo(*boost::static_pointer_cast<privateSharedMSI::cacheCoherenceInfo>(source)));
    return ret;
}

static boost::shared_ptr<void> directory_copy_coherence_info(boost::shared_ptr<void> source) {
    boost::shared_ptr<privateSharedMSI::directoryCoherenceInfo> ret
        (new privateSharedMSI::directoryCoherenceInfo(*boost::static_pointer_cast<privateSharedMSI::directoryCoherenceInfo>(source)));
    return ret;
}

static bool cache_is_hit(boost::shared_ptr<cacheRequest> req, cacheLine& line, const uint64_t& system_time) { 
    boost::shared_ptr<privateSharedMSI::cacheCoherenceInfo> info = 
        boost::static_pointer_cast<privateSharedMSI::cacheCoherenceInfo>(line.coherence_info);
...
static void directory_reserve_line(cacheLine &line) { 
    if (!line.coherence_info) {
        line.coherence_info = 
            boost::shared_ptr<privateSharedMSI::directoryCoherenceInfo>(new privateSharedMSI::directoryCoherenceInfo);
    }
    return; 
}
...
static void cache_reserve_line(cacheLine &line) { 
    if (!line.coherence_info) {
        line.coherence_info = 
            boost::shared_ptr<privateSharedMSI::cacheCoherenceInfo>(new privateSharedMSI::cacheCoherenceInfo);
    }
    boost::shared_ptr<privateSharedMSI::cacheCoherenceInfo> info = 
        boost::static_pointer_cast<privateSharedMSI::cacheCoherenceInfo>(line.coherence_info);
...
static bool directory_can_evict_line(cacheLine &line, const uint64_t &syste_time) { 
    boost::shared_ptr<privateSharedMSI::directoryCoherenceInfo> info = 
        boost::static_pointer_cast<privateSharedMSI::directoryCoherenceInfo>(line.coherence_info);
    return info->directory.empty();
}

privateSharedMSI::privateSharedMSI(uint32_t id, 
                                   const uint64_t &t, 
                                   boost::shared_ptr<tile_statistics> st, 
                                   logger &l, 
                                   boost::shared_ptr<random_gen> r, 
                                   boost::shared_ptr<cat> a_cat, 
    ...
    m_l2(NULL), 
    m_cat(a_cat), 
    m_stats(boost::shared_ptr<privateSharedMSIStatsPerTile>()),
...
void privateSharedMSI::request(boost::shared_ptr<memoryRequest> req) {
...
void privateSharedMSI::l1_work_table_update() {
    for (toL1Table::iterator it_addr = m_l1_work_table.begin(); it_addr != m_l1_work_table.end(); ) {
        ...
        boost::shared_ptr<toL1Entry> entry = it_addr->second;
        boost::shared_ptr<memoryRequest> core_req = entry->core_req;
        boost::shared_ptr<cacheRequest> l1_req = entry->l1_req;
        boost::shared_ptr<catRequest> cat_req = entry->cat_req;
        boost::shared_ptr<coherenceMsg> dir_req = entry->dir_req;
        boost::shared_ptr<coherenceMsg> dir_rep = entry->dir_rep;
        boost::shared_ptr<coherenceMsg> cache_req = entry->cache_req;
        boost::shared_ptr<coherenceMsg> cache_rep = entry->cache_rep;

        boost::shared_ptr<cacheLine> line = l1_req ? l1_req->line_copy() : boost::shared_ptr<cacheLine>();
        boost::shared_ptr<cacheLine> victim = l1_req ? l1_req->victim_line_copy() : boost::shared_ptr<cacheLine>();
        boost::shared_ptr<cacheCoherenceInfo> line_info = line ? 
            boost::static_pointer_cast<cacheCoherenceInfo>(line->coherence_info) : 
            boost::shared_ptr<cacheCoherenceInfo>();
        boost::shared_ptr<cacheCoherenceInfo> victim_info = victim ? 
            boost::static_pointer_cast<cacheCoherenceInfo>(victim->coherence_info) : 
            boost::shared_ptr<cacheCoherenceInfo>();

        //LOG(log,4) << "## L1 table " << m_id << " @ " << system_time << " on address " << start_maddr << " state " << entry->status << endl;
        ...
        if (entry->status == _L1_WORK_READ_L1) {
            ...
            if (dir_req) {
                ...
                if (l1_req->status() == CACHE_REQ_MISS) {
                    ...
                    LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] discarded a directory request for "
                    ...
                    cache_rep = boost::shared_ptr<coherenceMsg>(new coherenceMsg);
                    ...
                    if (dir_home == m_id) {
                        ...
                        boost::shared_ptr<message_t> new_msg(new message_t);
                        new_msg->src = m_id;
                ...
                if (l1_req->status() == CACHE_REQ_HIT) {
                    ...
                    boost::shared_array<uint32_t> ret(new uint32_t[core_req->word_count()]);
                    ...
                    LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] gets a HIT and finish serving address " 
                        << core_req->maddr() << endl;
                    m_l1_work_table.erase(it_addr++);
                    continue;
                } else {
                    /* miss */
                    if (line && line->valid) {
                        LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] gets a coherence miss on " << start_maddr 
                                  << endl;
                        mh_assert(line_info->status == SHARED && !core_req->is_read());
                        l1_req = boost::shared_ptr<cacheRequest>(new cacheRequest(start_maddr, CACHE_REQ_INVALIDATE));
                        ...
                        if (victim) {
                            LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] evicted a line for " << victim->start_maddr
                            ...
                            cache_rep->data = (data_size > 0) ? victim->data : boost::shared_array<uint32_t>();
                            cache_rep->did_win_last_arbitration = false;
                            entry->cache_rep = cache_rep;
                            if (dir_home == m_id) {
                                m_to_directory_rep_schedule_q.push_back(cache_rep);
            ...
            cache_rep->type = INV_REP;
            cache_rep->maddr = start_maddr;
            cache_rep->data = boost::shared_array<uint32_t>();
            cache_rep->did_win_last_arbitration = false;
            cache_rep->waited = 0; /* for debugging only - erase later */
            entry->cache_rep = cache_rep;
            if (dir_home == m_id) {
            ...
            if (cache_rep->did_win_last_arbitration) {
                if (entry->net_msg_to_send) {
                    entry->net_msg_to_send = boost::shared_ptr<message_t>();
                }
                cache_rep->did_win_last_arbitration = false;
                if (dir_req) {
                    /* this entry is due to a directory request. sending a reply finishes the job. */
                    LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] has sent a cache reply to directory "
            ...
            if (dir_rep) {
                ...
                line_info = boost::shared_ptr<cacheCoherenceInfo>(new cacheCoherenceInfo);
                line_info->directory_home = dir_rep->sender;
                line_info->status = dir_rep->type == SH_REP? SHARED : MODIFIED;
                boost::shared_array<uint32_t> data = dir_rep->data;
                if (!core_req->is_read()) {
                ...
                l1_req = boost::shared_ptr<cacheRequest>(new cacheRequest(start_maddr, CACHE_REQ_UPDATE,
            ...
            ++m_l1_work_table_vacancy;
            ++m_available_core_ports;
            LOG(log,4) << "[L1 " << m_id << " @ " << system_time << " ] resolved a miss and finish serving address " 
                << core_req->maddr() << endl;
            m_l1_work_table.erase(it_addr++);
            continue;
        }
...
void privateSharedMSI::process_cache_rep(boost::shared_ptr<cacheLine> line, boost::shared_ptr<coherenceMsg> rep) {

    uint32_t sender = rep->sender;
    boost::shared_ptr<directoryCoherenceInfo> info = 
        boost::static_pointer_cast<directoryCoherenceInfo>(line->coherence_info);

    LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] received a cache rep from sender : " << sender 
...
void privateSharedMSI::l2_work_table_update() {
    for (toL2Table::iterator it_addr = m_l2_work_table.begin(); it_addr != m_l2_work_table.end(); ) {
        maddr_t start_maddr = it_addr->first;
        boost::shared_ptr<toL2Entry> entry = it_addr->second;
        boost::shared_ptr<cacheRequest> l2_req = entry->l2_req;
        boost::shared_ptr<coherenceMsg> cache_req = entry->cache_req;
        boost::shared_ptr<coherenceMsg> cache_rep = entry->cache_rep;
        boost::shared_ptr<coherenceMsg> dir_rep = entry->dir_rep;
        boost::shared_ptr<dramMsg> dram_req = entry->dram_req;
        boost::shared_ptr<dramMsg> dram_rep = entry->dram_rep;
        boost::shared_ptr<cacheLine> line = l2_req->line_copy();
        boost::shared_ptr<directoryCoherenceInfo> line_info = 
            line ? boost::static_pointer_cast<directoryCoherenceInfo>(line->coherence_info) : boost::shared_ptr<directoryCoherenceInfo>();;
        boost::shared_ptr<cacheLine> victim = l2_req->victim_line_copy();
        boost::shared_ptr<directoryCoherenceInfo> victim_info = 
            victim ? boost::static_pointer_cast<directoryCoherenceInfo>(victim->coherence_info) : boost::shared_ptr<directoryCoherenceInfo>();;

        //LOG(log,4) << "## L2 table " << m_id << " @ " << system_time << " on address " << start_maddr << " state " << entry->status << endl;
        ...
        if (entry->status == _L2_WORK_READ_L2) {
            ...
            if (!cache_req) {
                ...
                l2_req = boost::shared_ptr<cacheRequest>(new cacheRequest(start_maddr, CACHE_REQ_UPDATE, 
                ...
                LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " received an empty request for " << start_maddr << endl;
                ...
                if (l2_req->status() == CACHE_REQ_HIT) {
                    ...
                    if (line) {
                        if (victim) {
                            LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] evicted a line for " 
                                      << victim->start_maddr << " to make a space for " << start_maddr << endl;
                        }
                        if (victim && victim->dirty) {
                            dram_req = boost::shared_ptr<dramMsg>(new dramMsg);
                            dram_req->sender = m_id;
                            dram_req->receiver = m_dram_controller_location;
                            dram_req->req = boost::shared_ptr<dramRequest>(new dramRequest(victim->start_maddr,
                                                                                    DRAM_REQ_WRITE,
                                                                                    m_cfg.words_per_cache_line,
                                                                                    victim->data));
                            ...
                            if (m_dram_controller_location == m_id) {
                                ...
                                entry->net_msg_to_send = boost::shared_ptr<message_t>(new message_t);
                            ...
                            dram_req->sender = m_id;
                            dram_req->receiver = m_dram_controller_location;
                            dram_req->req = boost::shared_ptr<dramRequest>(new dramRequest(start_maddr,
                                                                                    DRAM_REQ_READ,
                                                                                    m_cfg.words_per_cache_line));
                            dram_req->did_win_last_arbitration = false;
                            entry->dram_req = dram_req;
                        ...
                        if (victim && m_l2_work_table.count(victim->start_maddr) == 0) {
                            LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] tries to invalidate caches of line for " 
                                      << start_maddr << " to evict the line " << endl;
                            boost::shared_ptr<coherenceMsg> new_msg(new coherenceMsg);
                            new_msg->sender = m_id;
                            new_msg->receiver = m_id;
                            new_msg->type = EMPTY_REQ;
                            new_msg->maddr = victim->start_maddr;
                            new_msg->data = boost::shared_array<uint32_t>();
            ...
            if (cache_rep) {
                process_cache_rep(line, cache_rep);
                entry->cache_rep = boost::shared_ptr<coherenceMsg>();
                if (line_info->directory.empty()) {
                    ...
                    if (line->dirty) {
                        ...
                        dram_req->sender = m_id;
                        dram_req->receiver = m_dram_controller_location;
                        dram_req->req = boost::shared_ptr<dramRequest>(new dramRequest(line->start_maddr,
                                                                                DRAM_REQ_WRITE,
                                                                                m_cfg.words_per_cache_line,
                                                                                line->data));
                        dram_req->did_win_last_arbitration = false;
            ...
            while (entry->dir_reqs.size()) {
                boost::shared_ptr<coherenceMsg> dir_req = entry->dir_reqs.front();
                if (dir_req->did_win_last_arbitration) {
                    LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] has sent a directory request "
            ...
            while (entry->dir_reqs.size()) {
                ...
                if (dir_req->did_win_last_arbitration) {
                    ...
                    LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] already received a cache reply "
                    ...
                    if (++dir_req->waited > 10000) {
                        LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] cannot send a directory request "
            ...
            if (l2_req->status() == CACHE_REQ_HIT) {
                uint32_t sender = cache_req->sender;
                dir_rep = boost::shared_ptr<coherenceMsg>(new coherenceMsg);
                dir_rep->sender = m_id;
                ...
                if (sender == m_id) {
                    ...
                    LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] sent a directory reply to "
            ...
            if (dram_req->did_win_last_arbitration) {
                ...
                LOG(log,4) << "[DIRECTORY " << m_id << " @ " << system_time << " ] sent a DRAM request for "
                          << dram_req->req->maddr() << endl;
            ...
            if (dram_rep) {
                ...
                line_info = boost::shared_ptr<directoryCoherenceInfo>(new directoryCoherenceInfo);
                ...
                l2_req = boost::shared_ptr<cacheRequest>(new cacheRequest(start_maddr,
                                                                   CACHE_REQ_UPDATE,
                                                                   m_cfg.words_per_cache_line,
...
void privateSharedMSI::dram_work_table_update() {
    for (toDRAMTable::iterator it_addr = m_dram_work_table.begin(); it_addr != m_dram_work_table.end(); ) {
        boost::shared_ptr<toDRAMEntry> entry = it_addr->second;
        if (entry->dram_req->req->status() == DRAM_REQ_DONE) {
            if (!entry->dram_rep) {
                entry->dram_rep = boost::shared_ptr<dramMsg>(new dramMsg);
            ...
            if (entry->dram_req->sender == m_id) {
                ...
                LOG(log,4) << "[DRAM " << m_id << " @ " << system_time << " ] has sent a dram rep for address " << entry->dram_rep->req->maddr()
                          << " to core " << m_id << endl;
                ...
                if (!entry->dram_rep->did_win_last_arbitration) {
                    ...
                    LOG(log,4) << "[DRAM " << m_id << " @ " << system_time << " ] has sent a dram rep for address " 
...
void privateSharedMSI::accept_incoming_messages() {
    ...
    while (m_core_receive_queues[MSG_DIRECTORY_REQ_REP]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_DIRECTORY_REQ_REP]->front();
        boost::shared_ptr<coherenceMsg> cc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
        if (cc_msg->did_win_last_arbitration) {
           ...
           continue;
            ...
            if (++cc_msg->waited > 10000) {
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] cannot receive a directory ";
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] cannot receive a directory ";
                ...
                LOG(log,4) << cc_msg->type 
                    << ") for " << cc_msg->maddr  
                    << " from " << msg->src << " cannot get in"  
                    << " (table:size " << m_l1_work_table.size() << " ) ";
                if (m_l1_work_table.count(cc_msg->maddr)) {
                    LOG(log,4) << "existing entry state : " << m_l1_work_table[cc_msg->maddr]->status << endl;
                    LOG(log,4) << "existing entry state : " << m_l1_work_table[cc_msg->maddr]->status << endl;
                } else {
                    LOG(log,4) << "table full" << endl;
                    LOG(log,4) << "table full" << endl;
                }
            }
            ...
            if (cc_msg->type == SH_REP || cc_msg->type == EX_REP) {
                ...
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] received a directory reply for " 
                          << cc_msg->maddr << endl;
    ...
    while (m_core_receive_queues[MSG_CACHE_REQ]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_CACHE_REQ]->front();
        boost::shared_ptr<coherenceMsg> cc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
        if (cc_msg->did_win_last_arbitration) {
            /* reset the flag for the next arbitration */
            cc_msg->did_win_last_arbitration = false;
...
#if 1
            if (++cc_msg->waited > 10000) {
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] cannot receive a cache req (" << cc_msg->type 
                    << ") for " << cc_msg->maddr  
                    << " from " << msg->src << " cannot get in"  
                ...
                if (m_l2_work_table.count(cc_msg->maddr)) {
                    LOG(log,4) << "existing entry state : " << m_l2_work_table[cc_msg->maddr]->status << endl;
                    LOG(log,4) << "existing entry state : " << m_l2_work_table[cc_msg->maddr]->status << endl;
                } else {
                    LOG(log,4) << "table full" << endl;
                    LOG(log,4) << "table full" << endl;
    ...
    while (m_core_receive_queues[MSG_CACHE_REP]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_CACHE_REP]->front();
        boost::shared_ptr<coherenceMsg> cc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
        if (cc_msg->did_win_last_arbitration) {
            /* reset the flag for the next arbitration */
            cc_msg->did_win_last_arbitration = false;
...
#if 1
            if (++cc_msg->waited > 10000) {
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] cannot receive a cache rep (" << cc_msg->type 
                    << ") for " << cc_msg->maddr  
                    << " from " << msg->src << " cannot get in"  
                    << " (table:size " << m_l2_work_table.size() << " ) ";
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] cannot receive a cache rep (" << cc_msg->type 
    ...
    while (m_core_receive_queues[MSG_DRAM_REQ]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_DRAM_REQ]->front();
        boost::shared_ptr<dramMsg> dram_msg = boost::static_pointer_cast<dramMsg>(msg->content);
    ...
    if (m_core_receive_queues[MSG_DRAM_REP]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_DRAM_REP]->front();
        boost::shared_ptr<dramMsg> dram_msg = boost::static_pointer_cast<dramMsg>(msg->content);
...
void privateSharedMSI::schedule_requests() {
    ...
    while (m_l1_work_table_vacancy > 0 && m_to_cache_req_schedule_q.size()) {
        ...
        if (is_core_req) {
            boost::shared_ptr<memoryRequest> req = 
                boost::static_pointer_cast<memoryRequest>(m_to_cache_req_schedule_q.front().get<1>());
            ...
            boost::shared_ptr<toL1Entry> new_entry(new toL1Entry);
            new_entry->status = _L1_WORK_READ_L1;
            new_entry->core_req = req;

            boost::shared_ptr<cacheRequest> l1_req
                (new cacheRequest(req->maddr(), 
                                  req->is_read()? CACHE_REQ_READ : CACHE_REQ_WRITE,
                                  req->word_count(),
                                  req->is_read()? boost::shared_array<uint32_t>() : req->data()));
            new_entry->l1_req = l1_req;
           
            boost::shared_ptr<catRequest> cat_req(new catRequest(req->maddr(), m_id));
            new_entry->cat_req = cat_req;

            new_entry->dir_req = boost::shared_ptr<coherenceMsg>();
            new_entry->dir_rep = boost::shared_ptr<coherenceMsg>();
            new_entry->cache_req = boost::shared_ptr<coherenceMsg>();
            new_entry->cache_rep = boost::shared_ptr<coherenceMsg>();
            new_entry->requested_time = system_time;
            new_entry->net_msg_to_send = boost::shared_ptr<message_t>();
            ...
            boost::shared_ptr<coherenceMsg> msg = 
                boost::static_pointer_cast<coherenceMsg>(m_to_cache_req_schedule_q.front().get<1>());
            ...
            if (m_l1_work_table.count(start_maddr)) {
                    ...
                    !(m_l1_work_table[start_maddr]->dir_rep))
                {
                    ...
                    LOG(log,4) << "[L1 " << m_id  << " @ " << system_time << " ] discarded a directory request (" << msg->type 
                        << ") for address " << msg->maddr << " (state: " << m_l1_work_table[start_maddr]->status << " ) " << endl;
                }
            } else if (m_l1_work_table_vacancy) {
                boost::shared_ptr<toL1Entry> new_entry(new toL1Entry);
                new_entry->status = _L1_WORK_READ_L1;
                new_entry->core_req = boost::shared_ptr<memoryRequest>();

                LOG(log,4) << "[L1 " << m_id  << " @ " << system_time << " ] received a directory request (" << msg->type 
                    << ") for address " << msg->maddr << endl;

                boost::shared_ptr<cacheRequest> l1_req;
                if (msg->type == WB_REQ) {
                    boost::shared_ptr<cacheCoherenceInfo> new_info(new cacheCoherenceInfo);
                    new_info->status = SHARED;
                    new_info->directory_home = msg->sender;
                    l1_req = boost::shared_ptr<cacheRequest>(new cacheRequest(msg->maddr, CACHE_REQ_UPDATE,
                                                                       0, boost::shared_array<uint32_t>(), new_info));
                } else {
                    /* invReq or flushReq */
                    l1_req = boost::shared_ptr<cacheRequest>(new cacheRequest(msg->maddr, CACHE_REQ_INVALIDATE));
                }
                l1_req->set_reserve(false);
                new_entry->l1_req = l1_req;

                new_entry->cat_req = boost::shared_ptr<catRequest>();
    ...
    while (m_to_directory_rep_schedule_q.size()) {
        boost::shared_ptr<coherenceMsg> msg = m_to_directory_rep_schedule_q.front();
        maddr_t start_maddr = msg->maddr;
        if (m_l2_work_table.count(start_maddr)) {
            if (m_l2_work_table[start_maddr]->accept_cache_replies && !m_l2_work_table[start_maddr]->cache_rep) {
                LOG(log,4) << "[MEM " << m_id << " @ " << system_time << " ] received a cache reply (" << msg->type 
            ...
            LOG(log,4) << "[MEM " << m_id << " @ " << system_time << " ] received a cache reply from " << msg->sender 
                << " for address " << msg->maddr << " (new entry - reserved) " << endl;
            boost::shared_ptr<toL2Entry> new_entry(new toL2Entry);
            ...
            boost::shared_ptr<cacheRequest> l2_req(new cacheRequest(msg->maddr, CACHE_REQ_READ, m_cfg.words_per_cache_line));
            l2_req->set_reserve(true);
            new_entry->l2_req = l2_req;

            new_entry->cache_req = boost::shared_ptr<coherenceMsg>();
            new_entry->dir_rep = boost::shared_ptr<coherenceMsg>();
            new_entry->dram_req = boost::shared_ptr<dramMsg>();
            new_entry->dram_rep = boost::shared_ptr<dramMsg>();
    ...
    while (m_to_directory_req_schedule_q.size()) {
        boost::shared_ptr<coherenceMsg> msg = m_to_directory_req_schedule_q.front();
        maddr_t start_maddr = msg->maddr;
        if (m_l2_work_table.count(start_maddr)) {
        ...
        if (msg->type == EMPTY_REQ && m_l2_work_table_vacancy_evict) {
            ...
            LOG(log,4) << "[MEM " << m_id << " @ " << system_time << " ] received a cache request from " << msg->sender 
                      << " for address " << msg->maddr << " (new entry) " << endl;
            boost::shared_ptr<toL2Entry> new_entry(new toL2Entry);
    ...
    while (m_to_dram_req_schedule_q.size()) {
        ...
        boost::shared_ptr<dramMsg> msg = m_to_dram_req_schedule_q.front();
        if (m_dram_controller->available()) {
            if (msg->req->is_read()) {
                mh_assert(!m_dram_work_table.count(msg->req->maddr()));
                boost::shared_ptr<toDRAMEntry> new_entry(new toDRAMEntry);
    ...
    while (m_l2->read_port_available() && m_l2_read_req_schedule_q.size()) {
        boost::shared_ptr<cacheRequest> req = m_l2_read_req_schedule_q.front();
        maddr_t start_maddr = get_start_maddr_in_line(req->maddr());
    ...
    while (m_l2->write_port_available() && m_l2_write_req_schedule_q.size()) {
        boost::shared_ptr<cacheRequest> req = m_l2_write_req_schedule_q.front();
        m_l2->request(req);
    ...
    for (uint32_t it_channel = 0; it_channel < NUM_MSG_TYPES; ++it_channel) {
        ...
        while (m_to_network_schedule_q[it_channel].size()) {
            boost::shared_ptr<message_t> msg = m_to_network_schedule_q[it_channel].front();
            if (m_core_send_queues[it_channel]->push_back(msg)) {
                LOG(log,4) << "[NET " << m_id << " @ " << system_time << " ] network msg gone " << m_id << " -> " << msg->dst << " type " << it_channel << " num flits " << msg->flit_count << endl;
                switch (it_channel) {
                case MSG_CACHE_REQ:
                case MSG_CACHE_REP:
