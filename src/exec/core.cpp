...
core::core(const pe_id &id, const uint64_t &t, boost::shared_ptr<id_factory<packet_id> > pif,
...
{ 
    ...
    for (uint32_t i = 0; i < m_number_of_msg_types; ++i) {
        m_in_msg_queues[i] = boost::shared_ptr<messageQueue> (new messageQueue(i, m_msg_queue_size));
        m_out_msg_queues[i] = boost::shared_ptr<messageQueue> (new messageQueue(i, m_msg_queue_size));
    }
...
void core::connect(boost::shared_ptr<bridge> net_bridge) throw(err) {
    m_net = net_bridge;
    boost::shared_ptr<vector<uint32_t> > qs = m_net->get_ingress_queue_ids();
    m_queue_ids.clear();
    copy(qs->begin(), qs->end(), back_insert_iterator<vector<uint32_t> >(m_queue_ids));
}

boost::shared_ptr<messageQueue> core::receive_queue(uint32_t type) {
    assert((uint32_t)type < m_number_of_msg_types);
    return m_in_msg_queues[type];
}

boost::shared_ptr<messageQueue> core::send_queue(uint32_t type) {
    assert((uint32_t)type < m_number_of_msg_types);
...
void core::tick_positive_edge() throw(err) {
    ...
    for (uint32_t it = 0; it < m_number_of_msg_types; ++it) {
        ...
        boost::shared_ptr<messageQueue> send_q = m_out_msg_queues[type];
        if (send_q->size() == 0) {
            continue;
        }
