...
public:
    ...
    /* Common core methods */
    virtual void connect(boost::shared_ptr<bridge> net_bridge) throw(err);
...
protected:
    ...
    boost::shared_ptr<messageQueue> receive_queue(uint32_t type);
    boost::shared_ptr<messageQueue> send_queue(uint32_t type);
    ...
    /* memory */
    boost::shared_ptr<memory> m_memory;
...
private:
    ...
    map<uint32_t/*msg type*/, boost::shared_ptr<messageQueue> > m_out_msg_queues;
    map<uint32_t/*msg type*/, boost::shared_ptr<messageQueue> > m_in_msg_queues;

    /* Network */
    boost::shared_ptr<bridge> m_net;
    vector<uint32_t> m_queue_ids;
    incoming_packets_t m_incoming_packets;
    boost::shared_ptr<id_factory<packet_id> > m_packet_id_factory;
