...
void catStripe::request(boost::shared_ptr<catRequest> req) {
    assert(available());
    if (m_num_ports > 0) {
        --m_num_free_ports;
    }
...
void catStripe::tick_negative_edge() {
    while (!m_entry_queue.empty() && m_entry_queue.front().available_time <= system_time) {
        boost::shared_ptr<catRequest> req = m_entry_queue.front().request;
...
void catStatic::request(boost::shared_ptr<catRequest> req) {
...
void catFirstTouch::request(boost::shared_ptr<catRequest> req) {
    assert(available());
    if (m_num_ports > 0) {
        --m_num_free_ports;
    }
