...
dramRequest::dramRequest(maddr_t maddr, dramReqType_t request_type, uint32_t word_count) :
    ...
    m_data(boost::shared_array<uint32_t>()), m_aux_data(boost::shared_ptr<void>()) 
{
    assert(m_request_type == DRAM_REQ_READ);
}

dramRequest::dramRequest(maddr_t maddr, dramReqType_t request_type, uint32_t word_count, boost::shared_array<uint32_t> wdata) :
    m_request_type(request_type), m_maddr(maddr), m_word_count(word_count), m_aux_word_size(0), m_status(DRAM_REQ_NEW), 
    m_data(wdata), m_aux_data(boost::shared_ptr<void>()) 
{
    assert(m_request_type == DRAM_REQ_WRITE);
}

dramRequest::dramRequest(maddr_t maddr, dramReqType_t request_type, uint32_t word_count, boost::shared_array<uint32_t> wdata,
                         uint32_t aux_word_size, boost::shared_ptr<void> aux_data) :
    m_request_type(request_type), m_maddr(maddr), m_word_count(word_count), m_aux_word_size(0), m_status(DRAM_REQ_NEW), 
    m_data(boost::shared_array<uint32_t>()), m_aux_data(aux_data) 
{
    assert(m_request_type == DRAM_REQ_WRITE);
}
...
dramController::dramController(uint32_t id, const uint64_t &t,
                               boost::shared_ptr<tile_statistics> st, logger &l, boost::shared_ptr<random_gen> r,
                               boost::shared_ptr<dram> connected_dram,
...
void dramController::request(boost::shared_ptr<dramRequest> req) {
    assert(available());
    req->m_status = DRAM_REQ_WAIT;
    boost::shared_ptr<entry_t> new_entry = boost::shared_ptr<entry_t>(new entry_t);
...
void dramController::dram_access_safe(boost::shared_ptr<dramRequest> req) {
    boost::unique_lock<boost::recursive_mutex> lock(m_dram->dram_mutex);
    dram_access(req);
}

void dramController::dram_access(boost::shared_ptr<dramRequest> req) {
    ...
    for (uint32_t it_word = 0; it_word != req->m_word_count; ++it_word) {        
        ...
        if (req->is_read() && it_word == 0) {
            ...
            req->m_data = boost::shared_array<uint32_t>(new uint32_t[req->m_word_count]);
        }
        if (m_dram->m_memory[space].count(start_address) == 0) {
            m_dram->m_memory[space][start_address] = boost::shared_array<uint32_t>(new uint32_t[WORDS_IN_DRAM_BLOCK]);
...
void dram::mem_read_instant(    uint32_t * dst, 
    ...
    for (uint32_t it_word = 0; it_word != mem_size; ++it_word) {
        ...
        if (m_memory[mem_space].count(start_address) == 0) {
            m_memory[mem_space][start_address] = boost::shared_array<uint32_t>(new uint32_t[WORDS_IN_DRAM_BLOCK]);
            for (uint32_t i = 0; i < WORDS_IN_DRAM_BLOCK; ++i) {
...
at startup. */
void dram::mem_write_instant(   boost::shared_ptr<mem> source,
                                uint32_t mem_space,
                                uint32_t mem_start,
                                uint32_t mem_size) {
    mem_size = mem_size/4; // mem_size was in bytes  
