...
public:
    ...
    /* normal write */
    dramRequest(maddr_t maddr, dramReqType_t request_type, uint32_t word_count, boost::shared_array<uint32_t> wdata);
    /* extended write */
    dramRequest(maddr_t maddr, dramReqType_t request_type, 
                uint32_t word_count, boost::shared_array<uint32_t> wdata, 
                uint32_t aux_word_size, boost::shared_ptr<void> aux_data);

    inline dramReqStatus_t status() { return m_status; }
    inline boost::shared_array<uint32_t> read() { return m_data; }
    inline boost::shared_ptr<void> read_aux() { return m_aux_data; }
...
private:
    ...
    boost::shared_ptr<void> m_aux_data;
...
private:
    typedef map<uint64_t/*start_address*/, boost::shared_array<uint32_t> > memSpace;
    typedef map<uint32_t/*mem space id*/, memSpace> memSpaces;
    typedef map<maddr_t, boost::shared_ptr<void> > auxMemSpaces;
...
public:
    ...
    void mem_write_instant(boost::shared_ptr<mem>, uint32_t, uint32_t, uint32_t);
};
...
public:
    ...
    void request(boost::shared_ptr<dramRequest> req);

    inline bool available() { return m_number_of_free_ports > 0; }

private:
    void dram_access(boost::shared_ptr<dramRequest> req);
    void dram_access_safe(boost::shared_ptr<dramRequest> req);
    ...
    typedef struct {
        ...
        boost::shared_ptr<dramRequest> request;
        uint32_t port;
        uint32_t remaining_latency_cycles;
        uint32_t remaining_words_to_transfer;
    } entry_t;

    typedef vector<boost::shared_ptr<entry_t> > entryQueue;
