...
cacheRequest::cacheRequest(maddr_t maddr, cacheReqType_t request_type, uint32_t word_count,
                           boost::shared_array<uint32_t> data_to_write, boost::shared_ptr<void> coherence_info_to_write) :
    m_request_type(request_type), m_maddr(maddr), m_word_count(word_count),
    m_status(CACHE_REQ_NEW), m_line_copy(boost::shared_ptr<cacheLine>()), 
...
cache::cache(uint32_t level, uint32_t id, const uint64_t &t, boost::shared_ptr<tile_statistics> st, logger &l, boost::shared_ptr<random_gen> r, 
...
boost::shared_ptr<cacheLine> cache::copy_cache_line(const cacheLine &line) {
    boost::shared_ptr<cacheLine> copy(new cacheLine);

    /* shallow copy */
    (*copy) = line;
    if (line.data) {
        copy->data = boost::shared_array<uint32_t>(new uint32_t[m_words_per_line]);
...
void cache::request(boost::shared_ptr<cacheRequest> req) {
...
void cache::tick_positive_edge() {
    ...
    for (reqTable::iterator it_q = m_req_table.begin(); it_q != m_req_table.end(); ) {
        ...
        boost::shared_ptr<cacheRequest> req = head.request;
        ...
        if (head.status == ENTRY_DONE) {
            ...
            if (m_cache.count(idx) == 0) {
                ...
                for (uint32_t it_way = 0; it_way < m_associativity; ++it_way) {
                    ...
                    m_cache[idx][it_way].data = boost::shared_array<uint32_t>(new uint32_t[m_words_per_line]);
                    m_cache[idx][it_way].coherence_info = boost::shared_ptr<void>();
                }
            }
            bool matched = false;
                        ...
                        case CACHE_REQ_INVALIDATE:
                            {
                                if (req->m_do_reserve) {
                                    ...
                                    m_lines_to_evict_and_reserve.insert(boost::make_tuple(idx, it_way, start_maddr));
                                } else {
                                    req->m_line_copy = copy_cache_line(line);
                                    req->m_line_copy->empty = true;
                                    req->m_line_copy->valid = false;
                                    m_lines_to_evict.insert(boost::make_tuple(idx, it_way));
                                }
                                break;
                            }
                        case CACHE_REQ_READ:
        ...
        if (head.status == ENTRY_DONE) {
            ...
            if (!evictables.empty()) {
                ...
                if (!m_helper_can_evict_line ||
                    ...
                    m_lines_to_evict_and_reserve.insert(boost::make_tuple(idx, victim_way, start_maddr));
                }
            }
            /* exit code */
            if (req->use_read_ports()) {
...
void cache::tick_negative_edge() {
    ...
    for (set<boost::tuple<uint32_t, uint32_t> >::iterator it = m_lines_to_evict.begin(); it != m_lines_to_evict.end(); ++it) {
        m_cache[it->get<0>()][it->get<1>()].empty = true;
    }
    m_lines_to_evict.clear();

    for (set<boost::tuple<uint32_t, uint32_t, maddr_t> >::iterator it = m_lines_to_evict_and_reserve.begin(); 
