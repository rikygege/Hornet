...
static boost::shared_ptr<void> copy_coherence_info(boost::shared_ptr<void> source) { 
    boost::shared_ptr<privateSharedLCC::coherenceInfo> ret
        (new privateSharedLCC::coherenceInfo(*boost::static_pointer_cast<privateSharedLCC::coherenceInfo>(source)));
    return ret;
}

static bool l1_is_hit(boost::shared_ptr<cacheRequest> req, cacheLine& line, const uint64_t& system_time) { 
    boost::shared_ptr<privateSharedLCC::coherenceInfo> info = 
        boost::static_pointer_cast<privateSharedLCC::coherenceInfo>(line.coherence_info);
...
static bool l2_is_hit(boost::shared_ptr<cacheRequest> req, cacheLine &line, const uint64_t& system_time) {
    boost::shared_ptr<privateSharedLCC::coherenceInfo> info =
        boost::static_pointer_cast<privateSharedLCC::coherenceInfo>(line.coherence_info);
...
static void cache_reserve_line(cacheLine &line) { 
    if (!line.coherence_info) {
        line.coherence_info = 
            boost::shared_ptr<privateSharedLCC::coherenceInfo>(new privateSharedLCC::coherenceInfo);
    }
    return; 
}
...
static uint32_t replacement_policy_nearest_expiration_and_random(vector<uint32_t>& evictables, 
    ...
    for (vector<uint32_t>::iterator i = evictables.begin(); i != evictables.end(); ++i) {
        boost::shared_ptr<privateSharedLCC::coherenceInfo> info = 
            boost::static_pointer_cast<privateSharedLCC::coherenceInfo>(lines[*i].coherence_info);
...
privateSharedLCC::privateSharedLCC(uint32_t id, 
    ...
    m_l2(NULL), 
    m_cat(a_cat), 
    m_stats(boost::shared_ptr<privateSharedLCCStatsPerTile>()),
...
uint64_t privateSharedLCC::get_expiration_time(boost::shared_ptr<cacheLine> line) {
    if (line) {
        boost::shared_ptr<coherenceInfo> info = boost::static_pointer_cast<coherenceInfo>(line->coherence_info);
        if (info) {
            if (info->synched_expiration_time) {
...
void privateSharedLCC::request(boost::shared_ptr<memoryRequest> req) {
...
//        cerr << "doing something for l1 " << m_id << " @ " << system_time << " for " << start_maddr << " state " << entry->status << endl;
        ...
        boost::shared_ptr<coherenceMsg> lcc_req = entry->lcc_req;
        boost::shared_ptr<coherenceMsg> lcc_rep = entry->lcc_rep; 
        boost::shared_ptr<message_t> msg_to_send = entry->net_msg_to_send;
        ...
        if (entry->status == _L1_WORK_WAIT_L1) {
            ...
            if (l1_req->status() == CACHE_REQ_HIT) {
                ...
                if (cat_req->status() == CAT_REQ_DONE) {
                    ...
                    lcc_req = boost::shared_ptr<coherenceMsg>(new coherenceMsg);
                    lcc_req->sender = m_id;
                    ...
                    lcc_req->data = core_req->is_read() ? boost::shared_array<uint32_t>() : core_req->data();
                    ...
                    if (dir_home == m_id) {
                        ...
                        msg_to_send = boost::shared_ptr<message_t>(new message_t);
            ...
            if (cat_req->status() == CAT_REQ_DONE) {
                ...
                if (core_req->is_read()) {
                    ...
                    lcc_req->maddr = start_maddr;
                    lcc_req->data = boost::shared_array<uint32_t>();
                } else {
            ...
            if (lcc_rep) {
                ...
                if (lcc_req->type == WRITE_REQ) {
                    ...
                    if (system_time < timestamp) {
                        boost::shared_ptr<coherenceInfo> line_info(new coherenceInfo);
...
//       cerr << "doing something for l2 " << m_id << " @ " << system_time << " for " << start_maddr << " state " << entry->status << endl;
        ...
        boost::shared_ptr<cacheLine> line = l2_req ? l2_req->line_copy() : boost::shared_ptr<cacheLine>();
        boost::shared_ptr<cacheLine> victim = l2_req ? l2_req->victim_line_copy() : boost::shared_ptr<cacheLine>();
        boost::shared_ptr<coherenceInfo> line_info = 
            line ? boost::static_pointer_cast<coherenceInfo>(line->coherence_info) : boost::shared_ptr<coherenceInfo>();
        boost::shared_ptr<coherenceInfo> victim_info = 
            victim ? boost::static_pointer_cast<coherenceInfo>(victim->coherence_info) : boost::shared_ptr<coherenceInfo>();

        boost::shared_ptr<coherenceMsg> lcc_read_req = entry->lcc_read_req;
        boost::shared_ptr<coherenceMsg> lcc_write_req = entry->lcc_write_req;
        ...
        if (entry->status == _L2_WORK_WAIT_L2) {
            ...
            if (l2_req->status() == CACHE_REQ_HIT) {
                ...
                if (lcc_write_req) {
                    ...
                    if (*line_info->first_read_time_since_last_expiration == UINT64_MAX) {
                        *line_info->first_read_time_since_last_expiration = system_time;
                        line_info->synched_expiration_time = boost::shared_ptr<uint64_t>(new uint64_t(UINT64_MAX));
                    }
                } else {
                    /* new timestamp */
                    bool renewed = false;
                ...
                lcc_rep = boost::shared_ptr<coherenceMsg>(new coherenceMsg);
                lcc_rep->sender = m_id;
            ...
            if (dram_rep) {
                ...
                if (m_cfg.save_timestamp_in_dram && dram_rep->req->read_aux()) {
                    line_info = boost::static_pointer_cast<coherenceInfo>(dram_rep->req->read_aux());
                    ...
                    line_info = boost::shared_ptr<coherenceInfo>(new coherenceInfo);
                    switch (m_cfg.logic) {
                    case TIMESTAMP_FIXED:
                        {
                            line_info->synched_expiration_time = boost::shared_ptr<uint64_t>();
                    ...
                    case TIMESTAMP_IDEAL:
                        {
                            line_info->synched_expiration_time = boost::shared_ptr<uint64_t>(new uint64_t(UINT64_MAX));
                            line_info->first_read_time_since_last_expiration = boost::shared_ptr<uint64_t>(new uint64_t(system_time));
                            if (lcc_write_req) {
                                *line_info->synched_expiration_time = system_time;
                                *line_info->first_read_time_since_last_expiration = UINT64_MAX;
                            }
                    ...
                    case TIMESTAMP_ZERO_DELAY:
                        {
                            ...
                            line_info->synched_expiration_time = boost::shared_ptr<uint64_t>();
                            line_info->timestamp_delta_small = boost::shared_ptr<uint64_t>(new uint64_t);
                            *line_info->timestamp_delta_small = DEFAULT_DELTA_SMALL;
                            line_info->timestamp_delta_large = boost::shared_ptr<uint64_t>(new uint64_t);
                            *line_info->timestamp_delta_large = DEFAULT_DELTA_LARGE;
                            line_info->in_large_mode = boost::shared_ptr<bool>(new bool(false));
                            line_info->expiration_time = 
            ...
            if (lcc_read_req) {
                ...
                if (lcc_rep->receiver == m_id) {
                    ...
                    entry->lcc_read_req = boost::shared_ptr<coherenceMsg>();
                } else {
...
void privateSharedLCC::accept_incoming_messages() {
    ...
    while (m_core_receive_queues[MSG_LCC_REP]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_LCC_REP]->front();
        boost::shared_ptr<coherenceMsg> lcc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
    ...
    while (m_core_receive_queues[MSG_LCC_REQ_1]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_LCC_REQ_1]->front();
        boost::shared_ptr<coherenceMsg> lcc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
    ...
    while (m_cfg.use_separate_vc_for_writes && m_core_receive_queues[MSG_LCC_REQ_2]->size()) {
        boost::shared_ptr<message_t> msg = m_core_receive_queues[MSG_LCC_REQ_2]->front();
        boost::shared_ptr<coherenceMsg> lcc_msg = boost::static_pointer_cast<coherenceMsg>(msg->content);
        if (lcc_msg->did_win_last_arbitration) {
            /* reset the flag for the next arbitration */
            lcc_msg->did_win_last_arbitration = false;
...
void privateSharedLCC::schedule_requests() {
    ...
    while (m_core_port_schedule_q.size()) {
        boost::shared_ptr<memoryRequest> req = m_core_port_schedule_q.front();
        ...
        if (req->is_read()) {
            ...
            boost::shared_ptr<cacheRequest> l1_req(new cacheRequest(req->maddr(), 
                                                             CACHE_REQ_READ,
                                                             req->word_count(),
        ...
        new_entry->lcc_rep = boost::shared_ptr<coherenceMsg>();
    ...
    while (m_to_l2_req_schedule_q.size()) {
        boost::shared_ptr<coherenceMsg> msg = m_to_l2_req_schedule_q.front();
        ...
        if (msg->type == WRITE_REQ) {
            if (m_l2_work_table_vacancy_shared > 0 && m_l2_work_table.count(start_maddr) == 0) {
                ...
                new_entry->lcc_read_req = boost::shared_ptr<coherenceMsg>();

                boost::shared_ptr<cacheRequest> l2_req(new cacheRequest(msg->maddr, CACHE_REQ_WRITE, 
                                                                 msg->word_count,
                                                                 msg->data));
                l2_req->set_reserve(true);
                l2_req->set_clean_write(false);
            ...
            if (m_l2_work_table.count(start_maddr)) {
                ...
                new_entry->lcc_write_req = boost::shared_ptr<coherenceMsg>();

                boost::shared_ptr<cacheRequest> l2_req(new cacheRequest(start_maddr, CACHE_REQ_READ, m_cfg.words_per_cache_line));
                l2_req->set_reserve(true);
                new_entry->l2_req = l2_req;
    ...
    while (m_to_l2_write_req_schedule_q.size()) {
        boost::shared_ptr<coherenceMsg> msg = m_to_l2_write_req_schedule_q.front();
        maddr_t start_maddr = get_start_maddr_in_line(msg->maddr);
        if (m_l2_work_table_vacancy_shared > 0 && m_l2_work_table.count(start_maddr) == 0) {
            mh_log(4) << "[MEM " << m_id << " @ " << system_time << " ] received an LCC write request from " << msg->sender 
                << " for address " << msg->maddr << " (new entry - reserved) " << endl;
    ...
    while (m_to_l2_read_req_schedule_q.size()) {
        boost::shared_ptr<coherenceMsg> msg = m_to_l2_read_req_schedule_q.front();
