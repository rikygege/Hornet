...
memtraceCore::memtraceCore(const pe_id &id, const uint64_t &t,
                           ...
                           boost::shared_ptr<tile_statistics> st, logger &l,
...
void memtraceCore::execute() {
        ...
        if (m_do_evict) {
            ...
            for (uint32_t i = 0; i < m_lanes.size(); ++i) {
                ...
                if ((m_lanes[cand].status == LANE_IDLE || m_lanes[cand].status == LANE_MIG)
                    ...
                    boost::shared_ptr<message_t> new_msg = boost::shared_ptr<message_t>(new message_t);
                    new_msg->src = get_id().get_numeric_id();
                    new_msg->dst = m_lanes[cand].thread->native_core();
                    new_msg->flit_count = m_flits_per_context;
                    new_msg->content = boost::shared_ptr<void>(new uint64_t);
                    *(boost::static_pointer_cast<uint64_t>(new_msg->content)) = (uint64_t)(m_lanes[cand].thread);
            ...
            if (cur.thread->remaining_alu_cycle() == 0) {
                ...
                if (cur.thread->type() == memtraceThread::INST_MEMORY) {
                    if (cur.thread->is_read()) {
                        cur.req = boost::shared_ptr<memoryRequest>(new memoryRequest(cur.thread->maddr(), cur.thread->word_count()));
                    } else {
                        /* this core model doesn't care data */
                        boost::shared_array<uint32_t> dummy = boost::shared_array<uint32_t>(new uint32_t[cur.thread->word_count()]);
                        cur.req = boost::shared_ptr<memoryRequest>(new memoryRequest(cur.thread->maddr(), cur.thread->word_count(), dummy));
                    }
    ...
    if (support_em()) {
        ...
        for (uint32_t i = 0; i < num_arrived_natives; ++i) {
            uint64_t content = *(boost::static_pointer_cast<uint64_t>(receive_queue(m_first_core_msg_type)->front()->content));
        ...
        for (uint32_t i = 0; i < num_arrived_guests && m_num_guests < max_guests; ++i) {
            uint64_t content = *(boost::static_pointer_cast<uint64_t>(receive_queue(m_first_core_msg_type+1)->front()->content));
            memtraceThread *new_thread = (memtraceThread*)content;
            load_thread(new_thread);
